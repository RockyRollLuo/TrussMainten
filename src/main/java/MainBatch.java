...
public class MainBatch {
    private static Logger LOGGER = Logger.getLogger(MainBatch.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int order = 1; order <= maxOrder; order++) {
            ...
            if (threadRange > 0) {
                ...
                Result result13 = Parallel.edgesInsertion(restGraph, dynamicEdges, trussMap_rest, maxThreadNum);
                result13.setDatasetName(datasetName);
                result13.setOrder(order);
            ...
            if (threadRange > 0) {
                for (int threadNum = 1; threadNum <= maxThreadNum; threadNum = threadNum * 2) {
                    Result result23 = Parallel.edgesDeletion(fullGraph, dynamicEdges, trussMap_full, threadNum);
                ...
                Result result23 = Parallel.edgesDeletion(fullGraph, dynamicEdges, trussMap_full, maxThreadNum);
                result23.setDatasetName(datasetName);
                result23.setOrder(order);
