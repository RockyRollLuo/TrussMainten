...
public class CoreDecomp {
    private static Logger LOGGER = Logger.getLogger(CoreDecomp.class);

    private Graph graph;

    public CoreDecomp(Graph graph) {
        this.graph = graph;
    }

    /**
     * compute the core of vertices in the given graph
        ...
        for (int t = 2; ; t++) {
            ...
            while (!delQueue.isEmpty()) {
                ...
                for (Integer w : setCommon) {
                    ...
                    int currentSup1 = supMap.get(e1)==null?0:supMap.get(e1);
                    supMap.put(e1, currentSup1 - 1);
                    if ((currentSup1 - 1) <= t - 2 && !delQueue.contains(e1)) {
                        delQueue.offer(e1);
                    }
                    int currentSup2 = supMap.get(e2)==null?0:supMap.get(e2);
